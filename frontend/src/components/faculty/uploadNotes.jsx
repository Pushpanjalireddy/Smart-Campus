...
/**
 * Component for faculty to upload lecture notes and study materials.
...
const UploadNotes = ({ onBack }) => {
    ...
    /**
     * Simulates the notes file submission process.
     */
    const handleUpload = () => {
        if (!selectedFile) {
            setUploadStatus({ type: 'error', message: 'Please select a notes or presentation file first.' });
            return;
        }
        ...
        setTimeout(() => {
            ...
            // Allow larger files for presentations/videos (e.g., 50MB)
            const MAX_SIZE = 50 * 1024 * 1024; 
            if (selectedFile.size > MAX_SIZE) { 
                setUploadStatus({ type: 'error', message: `File too large: ${selectedFile.name}. Max size is 50MB.` });
            } else {
                setUploadStatus({
                    type: 'success',
                    message: `Successfully uploaded notes file: ${selectedFile.name}!`
                });
    ...
    // Styling for the dropzone, using Purple (a855f7) for Notes theme
    const fileInputContainerStyle = {
        ...
        borderWidth: '2px',
        borderColor: selectedFile ? '#a855f7' : '#d1d5db',
        backgroundColor: selectedFile ? '#f3e8ff' : 'transparent', // bg-purple-50 : transparent
    };
    ...
    // Upload button uses Pink for action
        ...
        <div style={componentStyle}>
            <div style={cardStyle}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
                    <UploadCloud size={48} style={{ color: '#a855f7', margin: '0 auto', marginBottom: '0.75rem' }} /> {/* text-purple-500 */}
                    <h2 style={{ fontSize: '1.875rem', fontWeight: '800', color: '#1f2937' }}>Upload Lecture Notes</h2> {/* text-3xl font-extrabold */}
                    <p style={{ color: '#6b7280', marginTop: '0.5rem' }}> {/* text-gray-500 */}
                        Please upload notes, presentations, or supplementary materials.
                    </p>
                </div>
                ...
                <label
                    ...
                    // Simulate hover effect manually for better UX
                    onMouseOver={e => !selectedFile && (e.currentTarget.style.borderColor = '#c084fc', e.currentTarget.style.backgroundColor = '#f5f3ff')}
                ...
                >
                    ...
                    <input
                        ...
                        // Accept common document, presentation, and video types
                        accept=".pdf,.doc,.docx,.ppt,.pptx,.zip,.mp4,.mov"
                        style={{ display: 'none' }} 
                    />
                    ...
                    {selectedFile ? (
                        <div style={{ textAlign: 'center' }}>
                            ...
                            <p style={{ color: '#a855f7', fontWeight: '500', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '15rem', margin: '0 auto' }}>
                                {selectedFile.name}
                            </p>
                        ...
                        <div style={{ textAlign: 'center' }}>
                            ...
                            <p style={{ marginTop: '0.5rem', fontSize: '0.875rem', color: '#4b5563' }}>
                                <span style={{ fontWeight: '600', color: '#ec4899' }}>Click to upload</span> or drag and drop
                            </p>
                            <p style={{ fontSize: '0.75rem', color: '#6b7280' }}>PDF, PPTX, MP4, or ZIP (Max 50MB)</p>
                        </div>
                    )}
                </label>
                ...
                <button
                    ...
                    onMouseOver={(e) => {
                        if (selectedFile && !isLoading) e.currentTarget.style.backgroundColor = '#d31278'; // darker pink
                    }}
                    onMouseOut={(e) => {
